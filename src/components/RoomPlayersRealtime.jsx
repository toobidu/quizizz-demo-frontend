...
import DebugInfo from './roomPlayers/DebugInfo';
...
const RoomPlayersRealtime = ({ roomCode, initialPlayers = [] }) => {
  ...
  useEffect(() => {
    if (wsPlayers && wsPlayers.length > 0) {
      ...
      // Find new players
      const justJoinedPlayers = wsPlayers.filter(
        player => player.userId && !currentPlayerIds.includes(player.userId)
      );
  ...
  const handlePlayersUpdated = (normalizedPlayers, newHost, newTotalPlayers, newMaxPlayers) => {
    ...
    // Find new players
    const justJoinedPlayers = normalizedPlayers.filter(
      player => player.userId && !currentPlayerIds.includes(player.userId)
    );
  ...
  useRoomEvents({
    onPlayersUpdate: (data) => {
      console.log('Players updated in component (via useRoomEvents):', data);
    },

    onPlayerJoined: (playerData) => {
      console.log('Player joined event received in component (via useRoomEvents):', playerData);
    },
  ...
  return (
    <div className="room-players-realtime">
      {/* Room Header */}
      <RoomHeader
